...
public class SessionService {
    ...
    private static final String REDIS_KEY_PREFIX = "sid:";
    private static final String REDIS_FLD_USER_ID = "user_id";
    private static final String REDIS_FLD_CREATED_AT = "created_at";
    private static final String REDIS_FLD_UPDATED_AT = "updated_at";
    ...
    public boolean sessionExists(String sid) {
        return redis.hasKey(REDIS_KEY_PREFIX + sid);
    }
    ...
    public String createSession(String userId) {
        ...
        for (int attempt = 0; attempt < 5; attempt++) {
            ...
            String key = REDIS_KEY_PREFIX + sid;
            ...
            if (result == 1L) {
                if (userId != null) {
                    redis.opsForHash().put(key, REDIS_FLD_USER_ID, userId);
                }
                return sid;
            }
        }
    ...
    public String getUserId(String sid) {
        ...
        Object userId = redis.opsForHash().get(REDIS_KEY_PREFIX + sid, REDIS_FLD_USER_ID);
        return userId != null ? userId.toString() : null;
    }

    public void deleteSession(String sid) {
        if (sid != null && !sid.isEmpty()) {
            redis.delete(REDIS_KEY_PREFIX + sid);
        }
    }
    ...
    public void refreshSession(String sid) {
        String key = REDIS_KEY_PREFIX + sid;
        String now = DateTimeFormatter.ISO_INSTANT.format(Instant.now());
        redis.opsForHash().put(key, REDIS_FLD_UPDATED_AT, now);
